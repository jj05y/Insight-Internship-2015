...
public class ViewPagerAdapterVerify extends PagerAdapter {
    ...
    public ViewPagerAdapterVerify(ArrayList<ItemForVerification> items, Context c, RepsDatabaseHandler db, VerifyClassificationFragment parent) {
        this.items = items;
        this.c = c;
        this.db = db;
        mp = new MediaPlayer();
    ...
    public Object instantiateItem(ViewGroup container, int position) {
        ...
        int exercise = items.get(position).getExercise();
        ...
        actualText.setText(C.LABELS_EXERCISE_MAP.get(C.EXERCISES.get(exercise)).get(actualLabel));


        predictedText.setText(C.LABELS_EXERCISE_MAP.get(C.EXERCISES.get(exercise)).get(predictedLabel));

        TextView repCount = (TextView) myInflatedView.findViewById(R.id.text_verify_rep_numbers);
        repCount.setText("Verfiying Rep# " + (position+1) + " of " + items.size());
