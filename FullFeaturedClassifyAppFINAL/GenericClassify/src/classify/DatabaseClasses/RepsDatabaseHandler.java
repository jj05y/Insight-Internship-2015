...
public class RepsDatabaseHandler extends SQLiteOpenHelper {
    ...
    public RepsDatabaseHandler(Context context) {
    ...
    public ArrayList<ItemForClassify> getItemsForClassify(int exercise) {
        ...
        if (cursor.moveToFirst()) {
            do {
                ...
                list.add(new ItemForClassify(rowID,actualLabel,predictedLabel,name,featureString, rep, exercise));
    ...
    public ArrayList<ItemForVerification> getItemsForVerification(int exercise) {
        ...
        if (cursor.moveToFirst()) {
            do {
                ...
                String filename = cursor.getString(cursor.getColumnIndex(KEY_FILENAME));
                int rep = cursor.getInt(cursor.getColumnIndex(KEY_REP));
                list.add(new ItemForVerification(rowID, name, actualLabel, predictedLabel, filename, rep, exercise));
    ...
    public void updateExercise(int id, int exercise) {
        ...
        db.update(TABLE_DETAILS, values, KEY_ID + " = " + id, null);
        Log.d("spinner", "putting exercise in: " + C.EXERCISES.get(exercise) + " row number: " + id);
    }
    ...
    public int emptyDatabase() {
        SQLiteDatabase db= this.getWritableDatabase();
        int numRows = db.delete(TABLE_DETAILS, "1", null);
        db.close();
        return numRows;
    }


    public void removeAllPredictedLabels() {
        SQLiteDatabase db= this.getWritableDatabase();
        ContentValues values = new ContentValues();
        values.put(KEY_PREDICTED_LABEL, 0);
        db.update(TABLE_DETAILS, values, null, null);
        db.close();
    }
