...
public class RepFinder {
    ...
    private static int REP_END_STRETCH = 37;
    private static double FILM_STRETCH = 0.6;
    private static double FILM_DELAY = 2.2;
...
/*        if (flatSpots.isEmpty()) { //if there's no flat spots,,, return whole graph
    ...
    static public ArrayList<FilmSection> getTimes(double sampleRate, ArrayList<FlatSpot> flatSpots, String videoFile) {
        ...
        for (int i = 0; i < flatSpots.size() - 1; i++) {
            ...
            double repStart = ((double) flat.getEnd()) / sampleRate;
            double repEnd = ((double) nextFlat.getStart()) / sampleRate;
            double filmStart;
            double filmEnd;
